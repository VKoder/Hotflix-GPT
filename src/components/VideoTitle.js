const VideoTitle = ({original_title, overview}) => {
    return (
        <div className="w-screen aspect-video pt-[20%] lg:px-20 md:px-16 px-6 overflow-hidden bg-gradient-to-r from-black ">
            ...
            <button className="md:px-4 md:py-1.5 px-2 py-0.5 lg:px-4 lg:py-1.5 rounded-md text-black  bg-white hover:opacity-80 font-semibold "><i className="ri-play-fill pr-1 lg:text-lg md:text-base sm:text-xs text-[9px]" ></i>Play Now</button>
            <button className="md:px-4 md:py-1.5 px-2 py-0.5 lg:px-4 lg:py-1.5 rounded-md  text-white  bg-zinc-600 hover:opacity-80 font-semibold "><i className="ri-information-line pr-1 lg:text-xl  md:text-base sm:text-xs text-[9px]"></i>More Info</button>
            </div>
            </div>
        </div>
    )
